...
async def get_messages(match_id: str, user: CurrentUser, db: DbSession) -> list[MessageOut]:
    ...
    msgs = await list_messages(db, match, user.id)
...
async def post_message(
    ...
    """マッチ内にメッセージを送信する（REST フォールバック）。"""
    match = await _require_match(db, user, match_id)
    return await send_message(db, match, user.id, req.body)
...
async def chat_ws(
    ...
    try:
        while True:
            ...
            await manager.broadcast(match_uuid, msg)
